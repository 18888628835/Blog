...
下面我们重构上面的原始代码，可以看到由于逻辑混乱，上面的代码非常难读，我们首先按照不同的orderType分成不同的函数。
...
然后我们用一个after函数来实现切面，当函数返回next时，就会进入下一个处理逻辑

```
Function.prototype.after = function(fn) {
  const _self = this
  return function(...rest) {
    const result = _self.call(_self, ...rest)
    if (result === 'next') {
      return fn.call(...rest)
    }
    return result
