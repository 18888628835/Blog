...
* 在`package.json`中指定`scripts`与`config`(`config`属性会自动帮我们写好)
  ...
  设置好之后就能使用`npm run commit`代替`git commit`，按照提示步骤就可以规范地提交代码了。
...
解决方法是修改文件名`.cz-config.cjs`，这样会用`CJS`的方式来读。然后修改`config`属性：
...
我们希望能够自动帮我们生成`Change Log`，这里推荐使用`release-please`。

官方文档：[release-please-action](https://github.com/google-github-actions/release-please-action)

这是 github 出的工具，当我们将代码推送到 github 上时，可以根据我们的 `commit` 信息自动生成`Change Log`文件。
